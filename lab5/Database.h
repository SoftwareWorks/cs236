...
public:
	Database(Query temp1, DBmanager &temp2)		// this class is to 
	{
		...
		db = temp2;
		counter = temp2.getCounter();
		relations = db.getRelationlist();
		mygraph = db.getGraph();
		graph = temp2.printGraph();
		postorders = temp2.getPostorders();
		ruleorders = temp2.getRuleorders();
		backwardedges = temp2.getBackwardedges();
		evaluations = temp2.getEvaluations();
	}
...
//			cout << nonterminal[i] << endl;
	...
	//		output = output + "  " + current.first + ":" +  current.second.toStringset() + "\n";
	//	}
	//	output = output + "\n";
	//}
	...
	void reconstruct()
	{
		//output = "Schemes populated after " + int_to_string(counter) + " passes through the Rules.\n";
		...
		{
			output = output + qr.getQuerylist().at(i).toString() + "?\n\n";

			output = output + "  Postorder Numbers\n";

			//printPostorder(i);

			output = output + postorders[i];

			output = output + "\n";

			output = output + "  Rule Evaluation Order\n";

			//printRuleorder();

			output = output + ruleorders[i];

			output = output + "\n";

			output = output + "  Backward Edges\n";

			//output = output + mygraph.printBackward();
			output = output + backwardedges[i];

			output = output + "\n";

			output = output + "  Rule Evaluation\n";
			
			output = output + evaluations[i];
